...
def grafico_media_per_num_treno(conn, intervallo_date, scelta_linea_codice):
    data = conn.query(f"SELECT C.NumTreno AS NUMTRENO, AVG(rit) AS MEDIA_RIT FROM CORSE C, TRENI T WHERE C.NumTreno=T.NumTreno AND Linea={int(scelta_linea_codice)} AND Data>='{intervallo_date[0]}' AND Data<='{intervallo_date[1]}' GROUP BY C.NumTreno;", ttl=0)
